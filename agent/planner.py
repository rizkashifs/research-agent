...
Today's date is {current_date}. For latest, recent, current, news, prices,
schedules, versions, laws, or other time-sensitive questions, use search() and
state the date or recency of the evidence you found.
{internet_mode}
...
2. **search(query, max_results, timelimit)** - Search the web for up-to-date information (may return "No results" in offline mode). Use timelimit for explicitly recent questions.
...
def build_system_prompt(memory_context: str = "", internet_enabled: bool = True) -> str:
    ctx = f"\n## What you already know\n{memory_context}\n" if memory_context.strip() else ""
    internet_mode = ""
    if not internet_enabled:
        internet_mode = (
            "\nOffline mode is enabled. Do not rely on internet search for fresh facts. "
            "If the user asks for latest or current information, explain that web lookup "
            "is disabled and answer only from saved notes and model knowledge."
        )
